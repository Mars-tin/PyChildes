...
class TestPrepareChildes(unittest.TestCase):
    ...
    def test_process_unidentifiable(self):
        ...
        self.assertEqual(
            pc.process_unidentifiable('xxx yyy ww', config), '<unk> <unk> ww'
        )

        self.assertEqual(
            pc.process_unidentifiable('<CHI> xxx yyy', config), '<CHI> <unk> <unk>'
        )
    ...
    def test_process_basic(self):
        ...
        config = pc.ChatConfig(_TEST_CONFIG_PATH)
        self.assertEqual(
            pc.process_basic('how to do it ? 36000_38606', config), 'how to do it ?'
        )
        self.assertEqual(
            pc.process_basic('you need it „ right', config), 'you need it , right'
        )
        self.assertEqual(
            pc.process_basic('Mommy ‡ I want some', config), 'Mommy , I want some'
        )
        ...
        self.assertEqual(pc.process_basic('rhi^noceros', config), 'rhinoceros')
        self.assertEqual(
            pc.process_basic("I don't (..) know .", config), "I don't know ."
        )
        self.assertEqual(
            pc.process_basic("I don't (0.15) know .", config), "I don't know ."
        )
        self.assertEqual(
            pc.process_basic("I don't (1:05.15) know .", config), "I don't know ."
        )
        self.assertEqual(
            pc.process_basic('(...) what do you (...) think ?', config),
            'what do you think ?',
        )
        self.assertEqual(
            pc.process_basic('(13.4) what do you (2.) think ?', config),
            'what do you think ?',
        )
    ...
    def test_process_local_event(self):
        ...
        config = pc.ChatConfig(_TEST_CONFIG_PATH)
        self.assertEqual(
            pc.process_local_event('&=laughs', config), '<EVT> laughs <sep> <0> </EVT>'
        )
        self.assertEqual(
            pc.process_local_event('&=eats:cookie', config),
            '<EVT> eats cookie <sep> <0> </EVT>',
        )
        self.assertEqual(
            pc.process_local_event(
                '&{l=laughs and then continue until the end marked by &}l=laughs',
                config,
            ),
        ...
        self.assertEqual(
            pc.process_local_event(
                '&{n=waving:hands and then continue until the end marked by &}n=waving:hands',
                config,
            ),
    ...
    def test_process_linker(self):
        ...
        config = pc.ChatConfig(_TEST_CONFIG_PATH)
        self.assertEqual(
            pc.process_linker('smells good enough for +...', config),
        ...
        self.assertEqual(
            pc.process_linker('what did you +/.', config), 'what did you ...'
        )
        self.assertEqual(
            pc.process_linker('+, with your spoon.', config), 'with your spoon.'
        )
        self.assertEqual(
            pc.process_linker('++ he would have come', config), 'he would have come'
        )
        self.assertEqual(
            pc.process_linker("+^ I really didn't", config), "I really didn't"
        )
        self.assertEqual(
            pc.process_linker('+, with your spoon.', config), 'with your spoon.'
        )
        ...
        self.assertEqual(
            pc.process_linker('smells good enough for me +.', config),
            'smells good enough for me .',
        )
        self.assertEqual(
            pc.process_linker('and then the little bear said +"/.', config),
            'and then the little bear said :',
        )
        self.assertEqual(
            pc.process_linker("+\" if you do, I'll carry you on my back.", config),
            "if you do, I'll carry you on my back.",
        )
        self.assertEqual(
            pc.process_linker('the little bear said +".', config),
    ...
    def test_process_special_form(self):
        ...
        # real utterances
        self.assertEqual(
            pc.process_special_form('da_ba_da_ba_doo@b .', config), '<unk> .'
        )
        ...
        # real utterances
        self.assertEqual(
            pc.process_special_form('wakey_up@c (55.) .', config), '<unk> (55.) .'
        )
        self.assertEqual(
    ...
    def test_process_paralinguistic(self):
        ...
        config = pc.ChatConfig(_TEST_CONFIG_PATH)
        self.assertEqual(
            pc.process_paralinguistic('0 [=! laughs louder] .', config),
            '<EVT> laughs louder <sep> 0 </EVT> .',
        )
        self.assertEqual(
            pc.process_paralinguistic("that's mine [=! cries] .", config),
        ...
        self.assertEqual(
            pc.process_paralinguistic('watch out [= laughing] .', config),
            'watch <exp> laughing <sep> out </exp> .',
        )
        self.assertEqual(
            pc.process_paralinguistic('word [=! whispers] .', config),
            '<EVT> whispers <sep> word </EVT> .',
        )
        self.assertEqual(
            pc.process_paralinguistic(
                '<first phrase> [= explains] and then <second phrase> [=! whispers]',
                config,
            ),
            '<exp> explains <sep> first phrase </exp> and then <EVT> whispers <sep> second phrase </EVT>',
        )
        self.assertEqual(
            pc.process_paralinguistic('one [=! laughs] and two [= comments]', config),
        ...
        self.assertEqual(
            pc.process_paralinguistic('where does that [!] go ?', config),
        ...
        self.assertEqual(
            pc.process_paralinguistic('Use word [# 3.1] when needed', config),
        ...
        self.assertEqual(pc.process_paralinguistic('goed [: went]', config), 'went')
        self.assertEqual(pc.process_paralinguistic('goed [: went] [*]', config), 'went')
        self.assertEqual(
            pc.process_paralinguistic('we want <one or two> [=? one too]', config),
            'we want one or two',
        )
        self.assertEqual(
            pc.process_paralinguistic(
                "I really wish you wouldn't [% said with strong raising of eyebrows] do that",
                config,
            ),
            "I really wish you <exp> said with strong raising of eyebrows <sep> wouldn't </exp> do that",
        )
        self.assertEqual(
            pc.process_paralinguistic('<going away with my mommy> [?] ?', config),
            'going away with my mommy ?',
        )
        self.assertEqual(
            pc.process_paralinguistic('<&~diddliddleiddle> [>]', config),
            '&~diddliddleiddle',
        )
        self.assertEqual(
            pc.process_paralinguistic(
        ...
        self.assertEqual(
            pc.process_paralinguistic("<Mommy I don't like this> [<]", config),
            "Mommy I don't like this",
        )
        self.assertEqual(
            pc.process_paralinguistic(
                'and the <doggy was> [>1] really cute and it <had to go> [>2] into bed',
                config,
            ),
            'and the doggy was really cute and it had to go into bed',
        )
        self.assertEqual(
            pc.process_paralinguistic('+< they had to go in here', config),
        ...
        self.assertEqual(
            pc.process_paralinguistic("it's [x 3] it's (.) a &-um (.) dog", config),
            "it's (.) a &-um (.) dog",
        )
        self.assertEqual(
            pc.process_paralinguistic(
        ...
        self.assertEqual(
            pc.process_paralinguistic('The pig has not yet fed [^c err]', config),
            'The pig has not yet fed',
        )
        self.assertEqual(pc.process_paralinguistic(" I'll get it. [+ bch]", config), '')
        self.assertEqual(pc.process_paralinguistic('0. [+ trn]', config), '0.')
        self.assertEqual(
            pc.process_paralinguistic('not this one. [+ neg] [- req] [+ inc]', config),
            'not this one.',
        )
